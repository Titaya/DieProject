...
//	this.music.play();
	...
	this.music.play();
	this.music1 = this.add.sound("ms",0.5,false);
...
this.item = this.add.group();
	...
	this.time.events.add(120000,this.Timeout,this);
	...
	for(var i=0;i<this.Boss.hpE;i++){
		...
		this.hpE[i] = this.add.sprite(700-(50*i),1150,"HpE");
...
Level.prototype.addTime = function(){
		Time = this.add.sprite(50,-150,"Time");
		Time.anchor.set(0,0);
		Time.scale.set(15);
		Time.animations.add("go").play(0.2,false);
		Time.smoothed = false;
...
Level.prototype.addLost = function(){
...
Level.prototype.Timeout = function(){
	this.music.stop();
	this.music3.play();
	this.update(this.Lost.visible=true);
	
};	
Level.prototype.addWin = function(){
...
Level.prototype.createBoss = function() {
		Boss = this.add.sprite(250,1200,"Kahang");
...
Level.prototype.BossonCollidebullet = function(Boss,bullet){
		bullet.kill();
		this.music1.play();
		this.Boss.hpE--;
...
Level.prototype.playerColideEnemies = function(player,enemies){
			enemies.kill();
			this.player.hpP--;
...
Level.prototype.playerColideBoss = function(player,Boss){
	this.player.kill();
	this.update(this.Lost.visible=true);
	this.music.stop();
	this.music3.play();
...
Level.prototype.playerColideItem = function(player,item){
		item.kill();
		this.update(this.num++);

	};

Level.prototype.quitLost = function() {
	this.music.stop();
	this.music1.stop();
	this.music2.stop();
	this.music3.stop();
...
Level.prototype.quitWin = function() {
	this.music.stop();
	this.music1.stop();
	this.music2.stop();
	this.music3.stop();
